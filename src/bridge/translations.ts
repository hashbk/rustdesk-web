/**
 * English translations extracted from RustDesk src/lang/en.rs.
 *
 * Used by the bridge `translate` handler to satisfy
 * `getByName('translate', jsonEncode({locale, text}))` calls from the
 * Dart side (flutter/lib/web/bridge.dart).
 *
 * Keys that are not found fall back to the original text, matching the
 * behaviour of RustDesk's native `translate()` in src/lang.rs.
 */
